...
```{r FigS5, warning=FALSE,echo=F, fig.width=10,fig.height=8, message=F, fig.align="center", results=F}
library("adegenet")
library(vcfR)
library("ade4")
...
#current plot with pop colours at random
p75 <- ggplot(dudi.pca,aes(x=pc1, y=pc2, colour=pop, group=pop)) + geom_point() +stat_ellipse(level=0.9) + scale_color_manual(name="Pop",values=col) + coord_fixed(xlim=c(-7, 6), ylim=c(-5.5,5.5)) +
  xlab(paste0("PC1 ", pc1per, "%")) + ylab(paste0("PC2 ", pc2per, "%")) + theme_bw() + ggtitle("p-15, r-50, missing 25%: 148 individuals, 5592 SNPs")+
  theme(legend.position = "none",panel.grid = element_blank(), axis.text=element_text(size=10),axis.title=element_text(size=12,face="bold"), 
        plot.margin = unit(c(0.5,0.5,0.5,0.5), "lines"), plot.title=element_text(size=10,face="bold",hjust=0.5))
...
#current plot with pop colours at random
pr70 <- ggplot(dudi.pca,aes(x=pc1, y=pc2, colour=pop, group=pop)) + geom_point() +stat_ellipse(level=0.9) + scale_color_manual(name="Pop",values=col) + coord_fixed(xlim=c(-7, 6), ylim=c(-5.5,5.5)) +
...
#current plot with pop colours at random
pr60 <- ggplot(dudi.pca,aes(x=pc1, y=pc2, colour=pop, group=pop)) + geom_point() +stat_ellipse(level=0.9) + scale_color_manual(name="Pop",values=col) + coord_fixed(xlim=c(-7, 6), ylim=c(-5.5,5.5)) +
...
```{r FigS6, warning=FALSE,echo=F, fig.width=10,fig.height=10, message=F, results=F, fig.align="center", cache=T}
...
```{r FigS7, warning=FALSE,echo=F, fig.width=14,fig.height=14, message=F, fig.align="center"}
...
```{r FigS8, warning=FALSE,echo=F, fig.width=11,fig.height=5, message=F, results=F, fig.align="center"}
...
```{r FigS9, warning=FALSE,echo=F, fig.width=10,fig.height=6, message=F, results=F, fig.align="center"}
...
```{r FigS10, warning=FALSE,echo=F, fig.width=10,fig.height=10, message=F, results=F, fig.align="center"}
