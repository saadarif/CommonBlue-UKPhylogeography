...
#make the plots
p <- ggplot(both, aes(x = KmerFrequency, y = Count, group=group))+ geom_line(aes(colour=group))+scale_x_continuous(expand=c(0,0),limits = c(0, 4000),breaks = scales::pretty_breaks(n = 20), name="K-mer Frequency")+scale_y_continuous(trans = 'log10', limits=c(100,10000000), name=expression("Log"[10]*" K-mer Count"))
p <- p + theme_bw()
p <-p + theme(text = element_text(size=10),legend.title = element_blank(),legend.position = c(.8,.75),legend.text=element_text(size=14, face = "bold"))
p <- p + coord_fixed(ratio = 250)
