...
#Dec 2020
#This space mix analysis assess population frequencies rather than indviduals
...
counts <- read.table("populations.snps.filter2.0.25.recode.singlesnp.vcf.counts")
#sum counts for each population

counts <- t(sapply(split(counts, pop(z)), function(x) colSums(x) ))
size <- read.table("populations.snps.filter2.0.25.recode.singlesnp.vcf.size")
size <- t(sapply(split(size, pop(z)), function(x) colSums(x) ))
...
rownames(size) <- NULL
colnames(size) <- NULL
sm_data_pop <- list(allele.counts=as.matrix(counts), sample.size=as.matrix(size), population.coordinates=as.matrix(coords[,2:3]))
str(sm_data_pop)
...
run.spacemix.analysis(n.fast.reps = 5,
                      ...
                      counts = sm_data_pop$allele.counts,
                      sample.sizes = sm_data_pop$sample.size,
                      sample.covariance=NULL,
                      target.spatial.prior.scale=NULL,
                      source.spatial.prior.scale=NULL,
                      spatial.prior.X.coordinates = sm_data_pop$population.coordinates[,1],
                      spatial.prior.Y.coordinates = sm_data_pop$population.coordinates[,2],
                      round.earth = T,
                      long.run.initial.parameters=NULL,
                      k = nrow(sm_data_pop$allele.counts),
                      loci = ncol(sm_data_pop$sample.size),
                      ngen = 1e8,
                      printfreq = 1e5,
                      samplefreq = 1e5,
                      mixing.diagn.freq = 50,
                      savefreq = 1e7,
                      directory=paste0(stem_dir,"/SingleSnp_run1"),
                      prefix = "byPop_run1")
...
run.spacemix.analysis(n.fast.reps = 5,
                      ...
                      mixing.diagn.freq = 50,
                      savefreq = 1e7,
                      directory=paste0(stem_dir,"/SingleSnp_run"),
                      prefix = "byPop_noadmixture_run1")

#--------------------------------------------------------------------------------------
setwd("/media/data_disk/PROJECTS/Saad/CommonBlue/stacks.denovo/stacks_m4_M4_n4_new/populations.r50.p15_moh_0.65/SpaceMix/")

#trace plots
load("./neutSingleSnp_run1/byPop_noadmixture_run1_LongRun/byPop_noadmixture_run1_space_MCMC_output1.Robj")
...
#   sample covariance.
load("./neutSingleSnp_run1//byPop_noadmixture_run1_LongRun/byPop_noadmixture_run1_MCN.frequencies.list.Robj")
...
sample.colors <- c("purple3", "green3", "green3", "goldenrod2", "goldenrod2","green3","goldenrod2", "goldenrod2", "goldenrod2", "green3","goldenrod2", "green3", 
                   "goldenrod2","goldenrod2","purple3")# And now generate a sample map list using a 95% 
#   credible interval on parameter estimates without 
#   `burning' (i.e., discarding) any sampled iterations
#   of the MCMC.
example.spacemix.map.list <- make.spacemix.map.list(MCMC.output.file = "./neutSingleSnp_run1/byPop_run1_LongRun/byPop_run1_space_MCMC_output1.Robj",
                                                    geographic.locations = sm_data_pop$population.coordinates,
                                                    name.vector = coords$POP,
...
make.spacemix.map(spacemix.map.list = example.spacemix.map.list,
                  text=TRUE,
                  ellipses=T,
                  source.option=F, xlim=c(-10,1), ylim=c(49, 58))

example.spacemix.map.listm4 <- make.spacemix.map.list(MCMC.output.file = "./neutSingleSnp_run1//byPop_noadmixture_run1_LongRun/byPop_noadmixture_run1_space_MCMC_output1.Robj",
                                                      geographic.locations = sm_data_pop$population.coordinates,
                                                      name.vector = coords$POP,
                                                      color.vector = sample.colors,
                                                      quantile=0.95,
                                                      burnin=0)
make.spacemix.map(spacemix.map.list = example.spacemix.map.listm4,
                  text=TRUE,
                  ellipses=T,
                  source.option=F, xlim=c(-10,1), ylim=c(49, 58))

#----------------------------------------------------------
#summaries of nugget parameters and admixutre proportions
x <- t(apply(nugget, 1, quantile, probs=c(0.025,0.975)))
rownames(x) <- coords$POP
y <- t(apply(admix.proportions, 1, quantile, probs=c(0.025,0.975)))
rownames(y) <- coords$POP
